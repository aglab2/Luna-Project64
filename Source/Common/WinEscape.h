...
namespace WinEscape
{
    ...
    namespace Utf8
    {
        ...
        std::string OpenFileDialog(void* hwndOwner, bool fileMustExist, std::vector<Utf8Filter> filters = {}, const char* initialDir = nullptr);

        std::vector<std::string> OpenFilesDialog(void* hwndOwner, std::vector<Filter> filters = {}, const char* initialDir = nullptr);

        // Save-file dialog.
        // Returns the selected file path, or an empty string if cancelled.
        std::string SaveFileDialog(void* hwndOwner, std::vector<Utf8Filter> filters = {}, const char* defaultName = nullptr, const char* initialDir = nullptr);
